# -*- encoding: utf-8 -*-
from lib.helpers import jsonifySeminars, re, parse, relativedelta, CachedResult, tAdjust, requests, BeautifulSoup, NavigableString, Tag
    ...
    if content.name == 'em':
      ...
      try:
        ...
        if url[-4:] == 'html':
          ...
          soup = BeautifulSoup(abstractPage.content.replace(
...
def getSeminarInfo(seminar):
  return reduce(generateDescription, seminar.contents[:-1],
                {'title': '',
                 'abstract': ''})
...
def getEventList(soup):
  ...
  triplets = filter(admissibleTriplets, ((data[i], data[i + 1], data[i + 2])
                                         for i in range(len(data) - 2)))

  # We can accept empty abstracts but not empty titles
  events = filter(lambda s: s['title'] != " - ", chain.from_iterable(
      filter(lambda t: t != None, map(cleanTriplets, triplets))))

  return events


def jsonifyLAS(cache=CachedResult()):
  return jsonifySeminars("http://www.london-analysis-seminar.org.uk/",
                         getEventList, cache)
