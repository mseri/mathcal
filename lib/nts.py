# -*- encoding: utf-8 -*-
from lib.helpers import parse, relativedelta, Tag, CachedResult, jsonifySeminars
...
def getEventList(soup):
  ...
  couples = filter(admissibleCouples, ((data[i], data[i + 1])
                                       for i in range(len(data) - 2)))

  # We can accept empty abstracts but not empty titles
  events = filter(lambda ev: ev != None, map(cleanCouple, couples))
...
def jsonifyNTS(cache=CachedResult()):
  return jsonifySeminars(
      "http://www.homepages.ucl.ac.uk/~ucahsze/seminars.html", getEventList,
      cache)
